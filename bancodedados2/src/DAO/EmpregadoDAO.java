...
" s.datanasc AS s_datanasc, s.dno AS s_dno, s.superssn AS s_superssn, s.senha AS s_senha, d.numero AS d_numero, d.nome AS d_nome, d.gerssn AS d_gerssn, d.gerdatainicio AS d_gerdatainicio ";
    ...
    private final String SQL_WORKAHOLIC = BEFORECOND + "FROM cia.empregado AS e, cia.departamento AS d, cia.empregado AS s, cia.trabalha_em as tt where tt.essn = e.ssn AND e.superssn = s.ssn AND e.dno = d.numero GROUP BY e.ssn,s.ssn,d.numero HAVING sum(tt.horas) = (SELECT MAX(HORAS) from (SELECT SUM(tt.horas) AS HORAS, e.ssn FROM cia.trabalha_em as tt, cia.empregado as e WHERE tt.essn = e.ssn GROUP BY e.ssn) AS A);";
    private final String SQL_NOTWORKAHOLIC = BEFORECOND + "FROM cia.empregado AS e, cia.departamento AS d, cia.empregado AS s, cia.trabalha_em as tt where tt.essn = e.ssn AND e.superssn = s.ssn AND e.dno = d.numero GROUP BY e.ssn,s.ssn,d.numero HAVING sum(tt.horas) = (SELECT MIN(HORAS) from (SELECT SUM(tt.horas) AS HORAS, e.ssn FROM cia.trabalha_em as tt, cia.empregado as e WHERE tt.essn = e.ssn GROUP BY e.ssn) AS A);";
